...
public class HeartRateMeasurement {
	...
	ArrayList<Float> rrIntervals = null;
	int hrmval = 0;
	int eeval = -1;
	...
	public HeartRateMeasurement(byte[] value) {
		...
		if (isRrIntPresent(flags)) {
			rrIntervals = new ArrayList<Float>();
	...
	public String toString() {
		String rv = "";
		rv = String.format("HR=%d,EE= %d,Worn=%s,intervals=%d [%.3f]", hrmval,eeval,sensorWorn.toString(),rrIntervals==null?-1:rrIntervals.size(),rrIntervals==null?-1.0:rrIntervals.size()>0?rrIntervals.get(0):0.0);
		return rv;
	}
}
